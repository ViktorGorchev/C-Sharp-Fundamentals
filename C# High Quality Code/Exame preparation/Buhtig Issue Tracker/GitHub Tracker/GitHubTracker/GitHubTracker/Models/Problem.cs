namespace GitHubTracker.Models
{
    ...
    public class Problem
    {
        ...
        public override string ToString()
        {
            ...
            {
                issue.AppendFormat("Tags: {0}", string.Join(",", this.Tags.Distinct().OrderBy(t => t))).AppendLine();
                //foreach (var tag in this.Tags)
                //{
                //    var spiltTag = tag.Split('|').Distinct().ToArray();

                //    issue.AppendFormat("Tags: {0}", string.Join(",", spiltTag.OrderBy(t => t))).AppendLine();
                //}
